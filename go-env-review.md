...
**Date:** 2026-08-24 (re-based 2026-09-11 on `origin/master` `e0c4727`, the revision salt
deploys)
...
current Go (≥1.21) cache/toolchain conventions. Line references are to `gorun.go` at
`e0c4727`.
...
environment** (`gorun.go:442`) and then patching exactly one thing, the HOME/GOCACHE pair
(`gorun.go:447-459`). GOPATH, GOMODCACHE, GOENV, XDG_CACHE_HOME, GOTOOLCHAIN and even a
...
All of gorun's working files live under `targetDirBase`, default **`/var/tmp`**
(`gorun.go:112`; moved from `/tmp` in `00bc87f`): `perUserTmpDir` is
`/var/tmp/gorun-<host>-<uid>` (`gorun.go:177-182`), each script gets a subdirectory
holding its binary and `.lastRun`, and each build a per-PID subdirectory of that.

1. `env = os.Environ()` — everything the caller had (`gorun.go:442`).
2. The embedded `go.env` section is appended, so it wins over inherited values
   (last-entry-wins, matched by `getEnvVar`) (`gorun.go:441-445`).
3. If `GOCACHE` is unset: when `HOME` is empty or `/`, set `HOME=<perRunTmpDir>`;
   ...
   to `HOME=<perRunTmpDir>` (`gorun.go:447-459`).
4. `GOTMPDIR` is pointed at the script's directory so `go build` temporaries land
   alongside the binary and are aged out by `clean()` (`gorun.go:460-461`).
5. The per-run dir — including the module cache and build cache when HOME was redirected
   there — is deleted after the build, with a chmod walk to defeat the read-only module
   cache (`gorun.go:476-483`).
6. Before running, the binary and `.lastRun` are touched (`gorun.go:580-584`,
   `e0c4727`) because systemd-tmpfiles ages `/var/tmp` (30 days by default) and would
   otherwise delete binaries still in use. The compiled script itself then runs with the
   original untouched environment (`gorun.go:585`) — correct, since the script binary
   doesn't need the go env.
7. Concurrent builds of the same script by the same user are serialised by PID
   directories (`hasActiveBuild`/`waitForActiveBuilds`, `gorun.go:487-562`), and
   `clean()` (`gorun.go:592-639`) removes script dirs unused for `cleanDays` (default 14)
   plus, inside each script dir, per-PID and `go-build*` directories older than an hour.
   Neither touches the environment, but both shape where caches can safely live.

## Changes

These assume adoption of a root-owned, optional `/etc/gorun.conf` (see "Rationale and
trade-offs" below). Precedence throughout: command-line flags (`GORUN_ARGS` is ignored
once a config file exists, see decision 5) > embedded `go.env` (per-script, repeatable) >
...
2. **Make the build env deterministic in `compile()`.** Set
   ...
   unconditionally (keeping the existing `GOTMPDIR` alongside them); never consult
   inherited GOPATH/GOCACHE/GOMODCACHE/XDG_CACHE_HOME; leave HOME untouched (preserving
   `~/.netrc`/git credentials for GOPRIVATE). Delete the HOME/GOCACHE heuristic at
   `gorun.go:447-459` — including the EACCES/ENOENT trap at `gorun.go:452` — rather than
   fixing it. The embedded `go.env` section remains the per-script override. This makes
   the reported GOPATH leak structurally impossible.
3. **Create and guard the per-uid cache dirs.** `cache_base` defaults to
   `/var/cache/gorun` (packaged via `tmpfiles.d`/deployment tooling; fallback under
   `targetDirBase`, i.e. `/var/tmp/gorun-<host>-<uid>/`, when absent); per-uid subdirs
   0700, verified owned-by-euid after MkdirAll (the ssh `~/.ssh` pattern). `clean()`
   (`gorun.go:592-639`) must skip the `gocache`/`gomod` dirs *before both of its passes*:
   its build-dir sweep deletes numeric-named subdirectories older than an hour as stale
   PID dirs, and the Go build cache's shard directories (`00`..`ff`) are numeric — in the
   fallback layout they would be silently pruned. Drop the chmod walk at
   `gorun.go:476-483` — the module cache no longer lives under the deleted per-run
   directory.
...
5. **Rework `goBinaryPath()`** to use `go_bin` from the config first, then PATH, then the
   ...
   use the same env in `goVer()` (`gorun.go:373`) as in `compile()` so version checks and
   builds agree.
...
7. **Verify ownership/mode of `perUserTmpDir`** (the binary target area) before writing
   ...
   via the config so it starts from a root-owned parent (decision 6: both). Under
   `/var/cache` systemd-tmpfiles ageing no longer applies, so the touch workaround of
   `e0c4727` becomes unnecessary there (it stays: harmless, and still right for the
   `/var/tmp` fallback).
...
2. **Env passthrough:** the build env inherits the full `os.Environ()` — `GOPROXY`,
   ...
   embedded `go.env`); the existing `GOTMPDIR` setting stays alongside them.
3. **Cache layout:** keyed by numeric uid — `/var/cache/gorun/<uid>/{gocache,gomod}`
   ...
   per-user dir under `targetDirBase`: `/var/tmp/gorun-<host>-<uid>/{gocache,gomod}`.
4. **Failure semantics:** hard error in both cases — a `/etc/gorun.conf` that exists but
   ...
   arose.
5. **`GORUN_ARGS`** (revised 2026-09-14): honoured only on machines without
   `/etc/gorun.conf`. When a config file is present `GORUN_ARGS` is ignored entirely,
   with a one-line stderr warning if it was set, so site policy can never be overridden
   through the environment. Nobody uses `GORUN_ARGS` here, so the simpler rule wins over
   per-key precedence: no flag stripping, no record of which keys the file set — `Config`
   carries the built-in defaults, the file overlays them, and the flags take those values
   as their defaults, with argv parsed last.
...
**EACCES, not ENOENT**, so the `os.IsNotExist` check at `gorun.go:452` is false, no
...
`gorun.go:447-459` — including the EACCES/ENOENT trap at `gorun.go:452` — is deleted, not
fixed.
...
which is deleted after the build (`gorun.go:426`), every recompile re-downloads all
modules and — on Go ≥1.21 — potentially an entire toolchain (see scenario 8). The README
documents this as a gotcha rather than fixing it. Anything that *does* persist lives in
`/var/tmp`, which systemd-tmpfiles ages — hence the per-run touch of the binary and
`.lastRun` (`e0c4727`) to keep in-use binaries alive.

**Should:** Solved by construction: `nobody` gets `<cache_base>/<uid>/{gocache,gomod}`
like every other user — persistent (surviving reboots and untouched by tmpfiles ageing
when `cache_base` is `/var/cache/gorun`), no HOME rewriting, no re-download on recompile.
Two details: `clean()` (`gorun.go:592-639`) must skip the cache dirs — its stale-PID-dir
sweep would otherwise prune the build cache's numeric shard directories in the fallback
layout (see change 3) — and with the module cache no longer living under the deleted
per-run directory, the chmod walk at `gorun.go:476-483` can go (`GOFLAGS=-modcacherw`
remains the modern tool if a throwaway module cache is ever wanted).
...
(`gorun.go:402-420`) tries `runtime.GOROOT()` — empty in a `-trimpath` build unless a
...
### 9. Adjacent but important: temp directory trust

`perUserTmpDir` has a predictable name (`/var/tmp/gorun-<host>-<uid>`,
`gorun.go:177-182`) and `os.MkdirAll` (`gorun.go:266`) succeeds silently if another user
pre-created it. `/var/tmp`, like `/tmp`, is world-writable (sticky), so the compiled
binary may be written to and `exec`'d from a directory that an attacker owns — a real
privilege-escalation path when root runs goscripts. The same trust question applies to
any persistent cache added per scenario 4.
...
world-writable `/var/tmp`; point `targetDirBase` there too. The residual check stays but
is small: after MkdirAll of a per-uid directory, verify it is owned by the euid with mode
0700 (the ssh `~/.ssh` pattern), and refuse if not. The same applies to
...
`strings.Split(section, "\n")` on the `go.env` section (`gorun.go:444`) produces
